...
import { logo2, logo3 } from "./reusable/logo";
...
const Nav = ({ projectsRef, skillsRef, contactRef, fixedNav, active }) => {
  ...
  const mobileMenu = (
    <div
      className={
        show
          ? `${style.mobile_menu} ${style.show}`
          : `${style.mobile_menu} ${style.hide}`
      }
    >
  ...
  return (
    <nav
      className={`${style.nav__components} ${
        fixedNav ? style.fixed_nav : null
      }`}
    >
      ...
      <div
        className={show ? `${style.mobile} ${style.rotated}` : style.mobile}
        onClick={(e) => onClick(e)}
      >
        {navSvg}
      </div>
